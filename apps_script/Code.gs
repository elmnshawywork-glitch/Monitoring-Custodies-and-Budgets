...
var TABLES = {
  ...
  db_schools:    ['id','name','type','period','students','active','category'],
...
function seedSchools_(){
  ...
  insert_('db_schools',{id:badr, name:'مدرسة ابدأ للعلوم التقنية — بدر', type:'مدرسة', period:'2026/2027', students:253, active:'نعم', category:'school'});
  insert_('db_schools',{id:dam,  name:'مدرسة ابدأ للعلوم التقنية — دمياط', type:'مدرسة', period:'2026/2027', students:255, active:'نعم', category:'school'});
  var B = [
  ...
  // الجهات الأخرى: الشركة الداخلية + التدريب (عام/مهنى)
  var comp = uid_(), trg = uid_(), trv = uid_();
  insert_('db_schools',{id:comp, name:'الشركة — العهدة الداخلية', type:'الشركة', period:'2026/2027', students:0, active:'نعم', category:'company'});
  insert_('db_schools',{id:trg,  name:'التدريب العام', type:'تدريب', period:'2026/2027', students:0, active:'نعم', category:'training_general'});
  insert_('db_schools',{id:trv,  name:'التدريب المهنى', type:'تدريب', period:'2026/2027', students:0, active:'نعم', category:'training_vocational'});
  var COMP = [['مصاريف إدارية','أدوات مكتبية ومستهلكات',0],['مصاريف إدارية','ضيافة وبوفيه',0],['مصاريف إدارية','صيانة وأجهزة',0],['مصاريف إدارية','انتقالات ونثريات',0],['مصاريف إدارية','تسويق',0]];
  var TR = [['تكلفة التدريب','أجور المدربين',0],['تكلفة التدريب','قاعات وتجهيزات',0],['تكلفة التدريب','مواد ومستلزمات تدريبية',0],['تكلفة التدريب','ضيافة وبوفيه',0],['تكلفة التدريب','انتقالات',0],['تكلفة التدريب','شهادات واعتماد',0]];
  COMP.forEach(function(x){ insert_('db_lines',{school_id:comp, section:x[0], name:x[1], allocated:x[2], note:''}); });
  TR.forEach(function(x){ insert_('db_lines',{school_id:trg, section:x[0], name:x[1], allocated:x[2], note:''}); });
  TR.forEach(function(x){ insert_('db_lines',{school_id:trv, section:x[0], name:x[1], allocated:x[2], note:''}); });
}
...
function route_(action, req, user){
  switch(action){
    ...
    case 'importBudget': {
      requireManager_(user);
      var sidI = req.school_id;
      if (req.newSchool){ var sN = insert_('db_schools', Object.assign({active:'نعم', category:'school'}, req.newSchool)); sidI = sN.id; }
      readAll_('db_lines').forEach(function(l){ if(String(l.school_id)===String(sidI)) deleteById_('db_lines', l.id); });
      (req.lines||[]).forEach(function(x){ insert_('db_lines', {school_id:sidI, section:x.section||'', name:x.name||'', allocated:Number(x.allocated)||0, note:''}); });
      return {ok:true, school_id:sidI, count:(req.lines||[]).length};
    }
