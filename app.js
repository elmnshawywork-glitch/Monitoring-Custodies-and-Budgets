...
/* ============================================================
   ...
   ============================================================ */
const REMOTE = !!(CFG.API_URL && CFG.API_URL.trim());
...
function seedDB() {
  ...
  const db = {
    ...
    db_schools: [{ id: badr, name: "مدرسة ابدأ للعلوم التقنية — بدر", type: "مدرسة", period: "2026/2027", students: 253, active: "نعم" },
                 { id: dam, name: "مدرسة ابدأ للعلوم التقنية — دمياط", type: "مدرسة", period: "2026/2027", students: 255, active: "نعم" }],
    db_lines: [...badrLines, ...damLines],
    ...
    db_holders: [{ id: uid(), name: "أحمد سمير", title: "مدير مدرسة بدر" }, { id: uid(), name: "منى فؤاد", title: "منسق دمياط" }],
...
function call(action, payload, session) { return REMOTE ? remoteCall(action, payload, session) : localCall(action, payload, session); }
...
function App() {
  ...
  const act = async (action, payload) => {
    try { const r = await call(action, payload, session); await boot(session); return r; }
    ...
    <div className="stack">
      <div className="toolbar">
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <select value={sid} onChange={e => setSid(e.target.value)}>{data.schools.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select>
          {canCentral && <button className="btn primary sm" onClick={() => setModal({ type: "central", payload: { school_id: sid } })}>+ شراء مركزى</button>}
        </div>
      </div>
      <div className="panel">
  ...
  return (
    <div className="stack">
      ...
      {data.custodies.map(c => {
        ...
        (data.expenses || []).filter(e => String(e.custody_id) === String(c.id) && e.approval !== "rejected").forEach(e => ledger.push({ date: e.date, kind: "out", item: e.spend_item || lineName(data, e.line_id) || "صرف", desc: e.description, amount: Number(e.amount || 0), approval: e.approval, settled: yes(e.settled) }));
        ...
        return (
          <div key={c.id} className="cust-block">
  ...
  return (
    <div className="stack">
      <div className="toolbar" style={{ alignItems: "flex-end" }}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <select value={fc} onChange={e => setFc(e.target.value)}><option value="all">كل العهد</option>{data.custodies.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}</select>
...
function AdminSchools({ data, act, setModal }) {
  ...
  return (
    <div className="stack">
      <div className="panel">
        <div className="toolbar"><h3 className="panel-title" style={{ margin: 0 }}>المدارس / الجهات</h3><button className="btn primary sm" onClick={() => setModal({ type: "school" })}>+ مدرسة</button></div>
        <div className="chips-list" style={{ marginTop: 12 }}>{data.schools.map(s => <span key={s.id} className="cat-chip">{s.name}<button className="icon-btn" style={{ width: 24, height: 24 }} onClick={() => setModal({ type: "school", payload: s })}>✎</button></span>)}</div>
      </div>
      <div className="panel">
        <div className="toolbar"><div style={{ display: "flex", gap: 8, alignItems: "center" }}><span style={{ fontWeight: 700, color: "var(--navy)" }}>بنود موازنة:</span><select value={sid} onChange={e => setSid(e.target.value)}>{data.schools.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div><button className="btn primary sm" onClick={() => setModal({ type: "line", payload: { school_id: sid } })}>+ بند</button></div>
...
function Reports({ data, act, showToast }) {
  ...
  const views = [["tx", "الحركات"], ["byItem", "حسب البند"], ["balances", "أرصدة العهد"], ["budget", "تحليل الموازنة"], ["docs", "متابعة المستندات"]];
  return (
    <div className="stack">
      <div className="panel">
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "end" }}>
          ...
          <select value={school} onChange={e => setSchool(e.target.value)}><option value="all">كل المدارس</option>{data.schools.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select>
      ...
      {view === "balances" && <RepBalances data={data} calc={calc} flt={flt} cust={cust} school={school} />}
      {view === "budget" && <RepBudget data={data} flt={flt} school={school} />}
...
function RepBalances({ data, calc, flt, cust, school }) {
  const custodies = data.custodies.filter(c => (cust === "all" || String(c.id) === cust) && (school === "all" || String(c.school_id) === school));
...
function RepBudget({ data, flt, school }) {
  ...
  const schools = school === "all" ? data.schools : data.schools.filter(s => String(s.id) === school);
...
function CustodyModal({ data, initial, onSave, close }) {
  ...
  return <Modal title={initial ? "تعديل العهدة" : "عهدة جديدة"} close={close}>
    ...
    <F label="المدرسة"><select value={school_id} onChange={e => setSchool(e.target.value)}>{data.schools.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></F>
...
function CentralModal({ data, school_id, act, onDone, close }) {
  ...
  return <Modal title="تسجيل شراء مركزى (يُخصم من الموازنة مباشرة)" close={close}>
    <F label="المدرسة"><select value={sid} onChange={e => setSid(e.target.value)}>{data.schools.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></F>
...
function SchoolModal({ initial, onSave, close }) {
  const [name, setName] = useState(initial ? initial.name : ""); const [type, setType] = useState(initial ? initial.type : "مدرسة"); const [period, setPeriod] = useState(initial ? initial.period : "2026/2027"); const [students, setStudents] = useState(initial ? initial.students : "");
  const save = () => { if (!name.trim()) return; const s = { name: name.trim(), type, period, students: Number(students) || 0 }; if (initial) { s.id = initial.id; s.active = initial.active; onSave(s, false); } else onSave(s, true); };
  return <Modal title={initial ? "تعديل المدرسة" : "مدرسة جديدة"} close={close}>
    <F label="اسم المدرسة / الجهة"><input value={name} onChange={e => setName(e.target.value)} /></F>
    <div className="grid2"><F label="النوع"><select value={type} onChange={e => setType(e.target.value)}><option>مدرسة</option><option>مركز تدريب</option><option>مشروع تدريبى</option></select></F><F label="عدد الطلاب"><input type="number" value={students} onChange={e => setStudents(e.target.value)} /></F></div>
